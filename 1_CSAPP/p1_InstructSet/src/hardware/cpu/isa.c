
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "../../headers/cpu.h"
#include "../../headers/memory.h"
#include "../../headers/common.h"

/*======================================*/
/*      instruction set architecture    */
/*======================================*/

uint64_t ACTIVE_CORE;
core_t cores[NUM_CORES];
// data structures
typedef enum INST_OPERATOR
{
    INST_MOV,           // 0
    INST_PUSH,          // 1
    INST_POP,           // 2
    INST_LEAVE,         // 3
    INST_CALL,          // 4
    INST_RET,           // 5
    INST_ADD,           // 6
    INST_SUB,           // 7
    INST_CMP,           // 8
    INST_JNE,           // 9
    INST_JMP,           // 10
} inst_type_t;

typedef enum OPERAND_TYPE
{
    EMPTY,                  // 0
    IMM,                    // 1
    REG,                    // 2
    MEM_IMM,                // 3
    MEM_REG1,               // 4
    MEM_IMM_REG1,           // 5
    MEM_REG1_REG2,          // 6
    MEM_IMM_REG1_REG2,      // 7
    MEM_REG2_SCAL,          // 8
    MEM_IMM_REG2_SCAL,      // 9
    MEM_REG1_REG2_SCAL,     // 10
    MEM_IMM_REG1_REG2_SCAL  // 11
}operand_type_t;

typedef struct OPERAND_STRUCT
{
    operand_type_t   type;   // IMM, REG, MEM
    uint64_t    imm;    // immediate number
    uint64_t    scal;   // scale number to register 2
    uint64_t    reg1;   // main register
    uint64_t    reg2;   // register 2
}operand_t;

// local variables are allocated in stack in run-time
// we don't consider local STATIC variables
// ref: Computer Systems: A Programmer's Perspective 3rd
// Chapter 7 Linking: 7.5 Symbols and Symbol Tables
typedef struct INST_STRUCT
{
    inst_type_t    instruct;	// enum of operators. e.g. mov, call, etc.
    operand_t      src;			// operand src of instruction
    operand_t	   dst;			// operand dst of instruction
} inst_t;

/*======================================*/
/*      parse assembly instruction      */
/*======================================*/

// lookup table
static const char *reg_name_list[72] = {
    "%rax","%eax","%ax","%ah","%al",
    "%rbx","%ebx","%bx","%bh","%bl",
    "%rcx","%ecx","%cx","%ch","%cl",
    "%rdx","%edx","%dx","%dh","%dl",
    "%rsi","%esi","%si","%sih","%sil",
    "%rdi","%edi","%di","%dih","%dil",
    "%rbp","%ebp","%bp","%bph","%bpl",
    "%rsp","%esp","%sp","%sph","%spl",
    "%r8","%r8d","%r8w","%r8b",
    "%r9","%r9d","%r9w","%r9b",
    "%r10","%r10d","%r10w","%r10b",
    "%r11","%r11d","%r11w","%r11b",
    "%r12","%r12d","%r12w","%r12b",
    "%r13","%r13d","%r13w","%r13b",
    "%r14","%r14d","%r14w","%r14b",
    "%r15","%r15d","%r15w","%r15b",
};
// reflection
static uint64_t reflect_register(const char *reg_name, core_t *core)
{
    // lookup table
    reg_t *reg = &(core->reg);
    uint64_t reg_addr[72] = {
        (uint64_t)&(reg->rax),(uint64_t)&(reg->eax),(uint64_t)&(reg->ax),(uint64_t)&(reg->ah),(uint64_t)&(reg->al),
        (uint64_t)&(reg->rbx),(uint64_t)&(reg->ebx),(uint64_t)&(reg->bx),(uint64_t)&(reg->bh),(uint64_t)&(reg->bl),
        (uint64_t)&(reg->rcx),(uint64_t)&(reg->ecx),(uint64_t)&(reg->cx),(uint64_t)&(reg->ch),(uint64_t)&(reg->cl),
        (uint64_t)&(reg->rdx),(uint64_t)&(reg->edx),(uint64_t)&(reg->dx),(uint64_t)&(reg->dh),(uint64_t)&(reg->dl),
        (uint64_t)&(reg->rsi),(uint64_t)&(reg->esi),(uint64_t)&(reg->si),(uint64_t)&(reg->sih),(uint64_t)&(reg->sil),
        (uint64_t)&(reg->rdi),(uint64_t)&(reg->edi),(uint64_t)&(reg->di),(uint64_t)&(reg->dih),(uint64_t)&(reg->dil),
        (uint64_t)&(reg->rbp),(uint64_t)&(reg->ebp),(uint64_t)&(reg->bp),(uint64_t)&(reg->bph),(uint64_t)&(reg->bpl),
        (uint64_t)&(reg->rsp),(uint64_t)&(reg->esp),(uint64_t)&(reg->sp),(uint64_t)&(reg->sph),(uint64_t)&(reg->spl),
        (uint64_t)&(reg->r8),(uint64_t)&(reg->r8d),(uint64_t)&(reg->r8w),(uint64_t)&(reg->r8b),
        (uint64_t)&(reg->r9),(uint64_t)&(reg->r9d),(uint64_t)&(reg->r9w),(uint64_t)&(reg->r9b),
        (uint64_t)&(reg->r10),(uint64_t)&(reg->r10d),(uint64_t)&(reg->r10w),(uint64_t)&(reg->r10b),
        (uint64_t)&(reg->r11),(uint64_t)&(reg->r11d),(uint64_t)&(reg->r11w),(uint64_t)&(reg->r11b),
        (uint64_t)&(reg->r12),(uint64_t)&(reg->r12d),(uint64_t)&(reg->r12w),(uint64_t)&(reg->r12b),
        (uint64_t)&(reg->r13),(uint64_t)&(reg->r13d),(uint64_t)&(reg->r13w),(uint64_t)&(reg->r13b),
        (uint64_t)&(reg->r14),(uint64_t)&(reg->r14d),(uint64_t)&(reg->r14w),(uint64_t)&(reg->r14b),
        (uint64_t)&(reg->r15),(uint64_t)&(reg->r15d),(uint64_t)&(reg->r15w),(uint64_t)&(reg->r15b),
    };

    for (int i = 0; i < 72; ++ i)
    {
        if (strcmp(reg_name, reg_name_list[i]) == 0)
        {
            return reg_addr[i];
        }
    }
    debug_fprintf(DEBUG_PARSE_INST, stderr, "parse operand %s\n    cannot parse register %s\n", reg_name);
    exit(0);
}

// functions to map the string assembly code to inst_t instance
static void parse_instruction(const char *str, inst_t *inst, core_t *core);
static void parse_operand(const char *str, operand_t *operand, core_t *core);
static uint64_t decode_operand(operand_t *operand);

// interpret the operand
static uint64_t decode_operand(operand_t *operand)
{
    if (operand->type == IMM)
    {
        // immediate signed number can be negative: convert to bitmap
        return *(uint64_t *)&operand->imm;
    }
    else if (operand->type == REG)
    {
        // default register 1
        return operand->reg1;
    }
    else if (operand->type == EMPTY)
    {
        return 0;
    }
    else
    {
        // access memory: return the physical address
        uint64_t vir_addr = 0;

        if (operand->type == MEM_IMM)
        {
            vir_addr = operand->imm;
        }
        else if (operand->type == MEM_REG1)
        {
            vir_addr = *((uint64_t *)operand->reg1);
        }
        else if (operand->type == MEM_IMM_REG1)
        {
            vir_addr = operand->imm + (*((uint64_t *)operand->reg1));
        }
        else if (operand->type == MEM_REG1_REG2)
        {
            vir_addr = (*((uint64_t *)operand->reg1)) + (*((uint64_t *)operand->reg2));
        }
        else if (operand->type == MEM_IMM_REG1_REG2)
        {
            vir_addr = operand->imm +  (*((uint64_t *)operand->reg1)) + (*((uint64_t *)operand->reg2));
        }
        else if (operand->type == MEM_REG2_SCAL)
        {
            vir_addr = (*((uint64_t *)operand->reg2)) * operand->scal;
        }
        else if (operand->type == MEM_IMM_REG2_SCAL)
        {
            vir_addr = operand->imm + (*((uint64_t *)operand->reg2)) * operand->scal;
        }
        else if (operand->type == MEM_REG1_REG2_SCAL)
        {
            vir_addr = (*((uint64_t *)operand->reg1)) + (*((uint64_t *)operand->reg2)) * operand->scal;
        }
        else if (operand->type == MEM_IMM_REG1_REG2_SCAL)
        {
            vir_addr = operand->imm + (*((uint64_t *)operand->reg1)) + (*((uint64_t *)operand->reg2)) * operand->scal;
        }
        return vir_addr;
    }
    
    // empty
    return 0;
}

static void parse_instruction(const char *str, inst_t *inst, core_t *core)
{
    char op_str[64] = {'\0'};
    int op_len = 0;
    char src_str[64] = {'\0'};
    int src_len = 0;
    char dst_str[64] = {'\0'};
    int dst_len = 0;

    char c;
    int count_parentheses = 0;
    int state = 0;

    for (int i = 0; i < strlen(str); ++ i)
    {
        c = str[i];
        if (c == '(' || c == ')')
        {
            count_parentheses ++;
        }

        if (state == 0 && c != ' ')
        {
            state = 1;
        }
        else if (state == 1 && c == ' ')
        {
            state = 2;
            continue;
        }
        else if (state == 2 && c != ' ')
        {
            state = 3;
        }
        else if (state == 3 && c == ',' && (count_parentheses == 0 || count_parentheses == 2))
        {
            state = 4;
            continue;
        }
        else if (state == 4 && c != ' ' && c != ',')
        {
            state = 5;
        }
        else if (state == 5 && c == ' ')
        {
            state = 6;
            continue;
        }

        if (state == 1)
        {
            op_str[op_len] = c;
            op_len ++;
            continue;
        }
        else if (state == 3)
        {
            src_str[src_len] = c;
            src_len ++;
            continue;
        }
        else if (state == 5)
        {
            dst_str[dst_len] = c;
            dst_len ++;
            continue;
        }
    }

    // op_str, src_str, dst_str
    // strlen(str)
    parse_operand(src_str, &(inst->src), core);
    parse_operand(dst_str, &(inst->dst), core);

    if (strcmp(op_str, "mov") == 0 || strcmp(op_str, "movq") == 0)
    {
        inst->instruct = INST_MOV;
    }
    else if (strcmp(op_str, "push") == 0)
    {
        inst->instruct = INST_PUSH;
    }
    else if (strcmp(op_str, "pop") == 0)
    {
        inst->instruct = INST_POP;
    }
    else if (strcmp(op_str, "leaveq") == 0)
    {
        inst->instruct = INST_LEAVE;
    }
    else if (strcmp(op_str, "callq") == 0)
    {
        inst->instruct = INST_CALL;
    }
    else if (strcmp(op_str, "retq") == 0)
    {
        inst->instruct = INST_RET;
    }
    else if (strcmp(op_str, "add") == 0)
    {
        inst->instruct = INST_ADD;
    }
    else if (strcmp(op_str, "sub") == 0)
    {
        inst->instruct = INST_SUB;
    }
    else if (strcmp(op_str, "cmpq") == 0)
    {
        inst->instruct = INST_CMP;
    }
    else if (strcmp(op_str, "jne") == 0)
    {
        inst->instruct = INST_JNE;
    }
    else if (strcmp(op_str, "jmp") == 0)
    {
        inst->instruct = INST_JMP;
    }

    debug_fprintf(DEBUG_PARSE_INST,stderr, "[%s (%d)] [%s (%d)] [%s (%d)]\n", op_str, inst->instruct, src_str, inst->src.type, dst_str, inst->dst.type);
}

// parse the string assembly operand to operand_t instance
static void parse_operand(const char *str, operand_t *operand, core_t *core)
{
    // str: the stripped compact operand string: turned to lower cases before parsing
    // od: the data structure to store operand
    operand->type = EMPTY;
    operand->imm = 0;
    operand->scal = 0;
    operand->reg1 = 0;
    operand->reg2 = 0;

    int str_len = strlen(str);
    if (str_len == 0)
    {
        // empty operand
        return;
    }    
    
    // parse the operand string
    if (str[0] == '$')
    {
        // immediate number
        operand->type = IMM;
        // try to parse the immediate number 64
        // condition short cut would not bring extra burden
        operand->imm = string2uint_range(str, 1, -1);
    }
    else if (str[0] == '%')
    {
        // register
        operand->type = REG;
        // match the correct register name
        operand->reg1 = reflect_register(str, core);
        return;
    }
    else
    {
        // should be a memory format, but check it
        // split imm(reg1,reg2,scal)
        char imm[64] = {'\0'};
        int imm_len = 0;
        char reg1[8] = {'\0'};
        int reg1_len = 0;
        char reg2[8] = {'\0'};
        int reg2_len = 0;
        char scal[2] = {'\0'};
        int scal_len = 0;

        int count_parentheses = 0;
        int count_comma = 0;
        // scan
        for (int i = 0; i < str_len; ++ i)
        {
            if (str[i] == '(' || str[i] == ')')
            {
                count_parentheses ++;
                continue;
            }            
            else if (str[i] == ',')
            {
                count_comma ++;
                continue;
            }
            else
            {
                if (count_parentheses == 0)
                {
                    // imm
                    imm[imm_len] = str[i];
                    imm_len ++;
                }
                else if (count_parentheses == 1)
                {
                    if (count_comma == 0)
                    {
                        // ...(reg1
                        reg1[reg1_len] = str[i];
                        reg1_len ++;
                    }
                    else if (count_comma == 1)
                    {
                        // ...(...,reg2
                        reg2[reg2_len] = str[i];
                        reg2_len ++;
                    }
                    else if (count_comma == 2)
                    {
                        // ...(...,...,scal
                        scal[scal_len] = str[i];
                        scal_len ++;
                    }
                }
            }
        }

        // parse imm
        if (imm_len > 0)
        {
            operand->imm = string2uint(imm);
            if (count_parentheses == 0)
            {
                // imm
                operand->type = MEM_IMM;
                return;
            }
        }
        // parse scale
        if (scal_len > 0)
        {
            operand->imm = string2uint(scal);
            if (operand->scal != 1 && operand->scal != 2 && operand->scal != 4 && operand->scal != 8)
            {
                debug_fprintf(DEBUG_PARSE_INST,stderr, "parse operand %s\n    scale number %s must be 1,2,4,8\n", scal);
                exit(0);
            }
        }
        // parse reg1
        if (reg1_len > 0)
        {
            operand->reg1 = reflect_register(reg1, core);
        }
        // parse reg2
        if (reg2_len > 0)
        {
            operand->reg2 = reflect_register(reg2, core);
        }

        // set types
        if (count_comma == 0)
        {
            // (r)
            operand->type = MEM_REG1;
        }
        else if (count_comma == 1)
        {
            // (r,r)
            operand->type = MEM_REG1_REG2;
        }
        else if (count_comma == 2)
        {
            if (reg1_len == 0)
            {
                // (,r,s)
                operand->type = MEM_REG2_SCAL;
            }
            else
            {
                // (r,r,s)
                operand->type = MEM_REG1_REG2_SCAL;
            }
        }
        // bias 1 for MEM_IMM_[.*]
        if (imm_len > 0)
        operand->type ++;
    }
}

/*======================================*/
/*      instruction handlers            */
/*======================================*/

// insturction (sub)set
// In this simulator, the instructions have been decoded and fetched
// so there will be no page fault during fetching
// otherwise the instructions must handle the page fault (swap in from disk) first
// and then re-fetch the instruction and do decoding
// and finally re-run the instruction

static void mov_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void push_handler            (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void pop_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void leave_handler           (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void call_handler            (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void ret_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void add_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void sub_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void cmp_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void jne_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);
static void jmp_handler             (operand_t *src_od, operand_t *dst_od, core_t *cr);

// handler table storing the handlers to different instruction types
typedef void (*handler_t)(operand_t *, operand_t *, core_t *);
// look-up table of pointers to function
static handler_t handler_table[NUM_INSTRTYPE] = {
    &mov_handler,               // 0
    &push_handler,              // 1
    &pop_handler,               // 2
    &leave_handler,             // 3
    &call_handler,              // 4
    &ret_handler,               // 5
    &add_handler,               // 6
    &sub_handler,               // 7
    &cmp_handler,               // 8
    &jne_handler,               // 9
    &jmp_handler,               // 10
};

// update the rip pointer to the next instruction sequentially
static inline void next_rip(core_t *cr)
{
    // we are handling the fixed-length of assembly string here
    // but their size can be variable as true X86 instructions
    // that's because the operands' sizes follow the specific encoding rule
    // the risc-v is a fixed length ISA
    cr->rip = cr->rip + sizeof(char) * MAX_INSTRUCTION_CHAR;
}

// instruction handlers

static void mov_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    uint64_t dst = decode_operand(dst_od);

    if (src_od->type == REG && dst_od->type == REG)
    {
        // src: register
        // dst: register
        *(uint64_t *)dst = *(uint64_t *)src;
        next_rip(core);
        core->flags.__cpu_flag_value = 0;
        return;
    }
    else if (src_od->type == REG && dst_od->type >= MEM_IMM)
    {
        // src: register
        // dst: virtual address
        write64bits_dram(virAddr2phyAddr(dst, core),  *(uint64_t *)src, core );
        next_rip(core);
        core->flags.__cpu_flag_value = 0;
        return;
    }
    else if (src_od->type >= MEM_IMM && dst_od->type == REG)
    {
        // src: virtual address
        // dst: register
        *(uint64_t *)dst = read64bits_dram( virAddr2phyAddr(src, core), core);
        next_rip(core);
        core->flags.__cpu_flag_value = 0;
        return;
    }
    else if (src_od->type == IMM && dst_od->type == REG)
    {
        // src: immediate number (uint64_t bit map)
        // dst: register
        *(uint64_t *)dst = src;
        next_rip(core);
        core->flags.__cpu_flag_value = 0;
        return;
    }
}

static void push_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    // uint64_t dst = decode_operand(dst_od);

    if (src_od->type == REG)
    {
        // src: register
        // dst: empty
        (core->reg).rsp = (core->reg).rsp - 8;
        write64bits_dram( virAddr2phyAddr((core->reg).rsp, core),  *(uint64_t *)src,  core );
        next_rip(core);
        core->flags.__cpu_flag_value = 0;
        return;
    }
}

static void pop_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    // uint64_t dst = decode_operand(dst_od);
    
    if (src_od->type == REG)
    {
        // src: register
        // dst: empty
        uint64_t old_val = read64bits_dram( virAddr2phyAddr((core->reg).rsp, core), core );
        (core->reg).rsp = (core->reg).rsp + 8;
        *(uint64_t *)src = old_val;
        next_rip(core);
        core->flags.__cpu_flag_value = 0;
        return;
    }
}

static void leave_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    // movq %rbp, %rsp
    (core->reg).rsp = (core->reg).rbp;

    // popq %rbp
    uint64_t old_val = read64bits_dram(virAddr2phyAddr((core->reg).rsp, core), core );
    (core->reg).rsp = (core->reg).rsp + 8;
    (core->reg).rbp = old_val;
    next_rip(core);
    core->flags.__cpu_flag_value = 0;
}

static void call_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    // uint64_t dst = decode_operand(dst_od);

    // src: immediate number: virtual address of target function starting
    // dst: empty
    // push the return value
    (core->reg).rsp = (core->reg).rsp - 8;
    write64bits_dram(virAddr2phyAddr((core->reg).rsp, core), core->rip + sizeof(char) * MAX_INSTRUCTION_CHAR, core );
    // jump to target function address
    core->rip = src;
    core->flags.__cpu_flag_value = 0;
}

static void ret_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    // uint64_t src = decode_operand(src_od);
    // uint64_t dst = decode_operand(dst_od);

    // src: empty
    // dst: empty
    // pop rsp
    uint64_t ret_addr = read64bits_dram(virAddr2phyAddr((core->reg).rsp, core),core );
    (core->reg).rsp = (core->reg).rsp + 8;
    // jump to return address
    core->rip = ret_addr;
    core->flags.__cpu_flag_value = 0;
}

static void add_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    uint64_t dst = decode_operand(dst_od);

    if (src_od->type == REG && dst_od->type == REG)
    {
        // src: register (value: int64_t bit map)
        // dst: register (value: int64_t bit map)
        uint64_t val = *(uint64_t *)dst + *(uint64_t *)src;

        int val_sign = ((val >> 63) & 0x1);
        int src_sign = ((*(uint64_t *)src >> 63) & 0x1);
        int dst_sign = ((*(uint64_t *)dst >> 63) & 0x1);

        // set condition flags
        core->flags.CF = (val < *(uint64_t *)src); // unsigned
        core->flags.ZF = (val == 0);
        core->flags.SF = val_sign;
        core->flags.OF = (src_sign == 0 && dst_sign == 0 && val_sign == 1) || (src_sign == 1 && dst_sign == 1 && val_sign == 0);

        // update registers
        *(uint64_t *)dst = val;
        // signed and unsigned value follow the same addition. e.g.
        // 5 = 0000000000000101, 3 = 0000000000000011, -3 = 1111111111111101, 5 + (-3) = 0000000000000010
        next_rip(core);
        return;
    }
}

static void sub_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    uint64_t dst = decode_operand(dst_od);

    if (src_od->type == IMM && dst_od->type == REG)
    {
        // src: register (value: int64_t bit map)
        // dst: register (value: int64_t bit map)
        // dst = dst - src = dst + (-src)
        uint64_t val = *(uint64_t *)dst + (~src + 1);

        int val_sign = ((val >> 63) & 0x1);
        int src_sign = ((src >> 63) & 0x1);
        int dst_sign = ((*(uint64_t *)dst >> 63) & 0x1);

        // set condition flags
        core->flags.CF = (val > *(uint64_t *)dst); // unsigned

        core->flags.ZF = (val == 0);
        core->flags.SF = val_sign;
        
        core->flags.OF = (src_sign == 1 && dst_sign == 0 && val_sign == 1) || (src_sign == 0 && dst_sign == 1 && val_sign == 0);

        // update registers
        *(uint64_t *)dst = val;
        // signed and unsigned value follow the same addition. e.g.
        // 5 = 0000000000000101, 3 = 0000000000000011, -3 = 1111111111111101, 5 + (-3) = 0000000000000010
        next_rip(core);
        return;
    }
}

static void cmp_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    uint64_t dst = decode_operand(dst_od);

    if (src_od->type == IMM && dst_od->type >= MEM_IMM)
    {
        // src: register (value: int64_t bit map)
        // dst: register (value: int64_t bit map)
        // dst = dst - src = dst + (-src)
        uint64_t dval = read64bits_dram(virAddr2phyAddr(dst, core), core);
        uint64_t val = dval + (~src + 1);

        int val_sign = ((val >> 63) & 0x1);
        int src_sign = ((src >> 63) & 0x1);
        int dst_sign = ((dval >> 63) & 0x1);

        // set condition flags
        core->flags.CF = (val > dval); // unsigned

        core->flags.ZF = (val == 0);
        core->flags.SF = val_sign;
        
        core->flags.OF = (src_sign == 1 && dst_sign == 0 && val_sign == 1) || (src_sign == 0 && dst_sign == 1 && val_sign == 0);

        // signed and unsigned value follow the same addition. e.g.
        // 5 = 0000000000000101, 3 = 0000000000000011, -3 = 1111111111111101, 5 + (-3) = 0000000000000010
        next_rip(core);
        return;
    }
}

static void jne_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);

    // src_od is actually a instruction memory address
    // but we are interpreting it as an immediate number
    if (core->flags.ZF == 0)
    {
        // last instruction value != 0
        core->rip = src;
    }
    else
    {
        // last instruction value == 0
        next_rip(core);
    }
    core->flags.__cpu_flag_value = 0;
}

static void jmp_handler(operand_t *src_od, operand_t *dst_od, core_t *core)
{
    uint64_t src = decode_operand(src_od);
    core->rip = src;
    core->flags.__cpu_flag_value = 0;
}

// instruction cycle is implemented in CPU
// the only exposed interface outside CPU
void instruction_cycle(core_t *core)
{
    // FETCH: get the instruction string by program counter
    char inst_str[MAX_INSTRUCTION_CHAR + 10];
    readinst_dram(virAddr2phyAddr(core->rip, core), inst_str, core);

    debug_fprintf(DEBUG_INSTRUCTION_CYCLE,stderr, "%8lx    %s\n", core->rip, inst_str);

    // DECODE: decode the run-time instruction operands
    inst_t inst;
    parse_instruction(inst_str, &inst, core);
    
    // EXECUTE: get the function pointer or handler by the operator
    handler_t handler = handler_table[inst.instruct];
    // update CPU and memory according the instruction
    handler(&(inst.src), &(inst.dst), core);
}

void print_register(core_t *core)
{
    if ((DEBUG_VERBOSE_SET & DEBUG_REGISTERS) == 0x0)
    {
        return;
    }

    reg_t reg = core->reg;
    
    printf("rax = %16lx\trbx = %16lx\trcx = %16lx\trdx = %16lx\n",reg.rax, reg.rbx, reg.rcx, reg.rdx);
    printf("rsi = %16lx\trdi = %16lx\trbp = %16lx\trsp = %16lx\n",reg.rsi, reg.rdi, reg.rbp, reg.rsp);
    printf("rip = %16lx\n", core->rip);
    printf("CF = %u\tZF = %u\tSF = %u\tOF = %u\n",core->flags.CF, core->flags.ZF, core->flags.SF, core->flags.OF);
}

void print_stack(core_t *core)
{
    if ((DEBUG_VERBOSE_SET & DEBUG_PRINT_STACK) == 0x0)
    {
        return;
    }

    int n = 10;    
    uint64_t *high = (uint64_t*)&phy_mem[virAddr2phyAddr((core->reg).rsp, core)];
    high = &high[n];
    uint64_t va = (core->reg).rsp + n * 8;
    
    for (int i = 0; i < 2 * n; ++ i)
    {
        uint64_t *ptr = (uint64_t *)(high - i);
        printf("0x%16lx : %16lx", va, (uint64_t)*ptr);

        if (i == n)
        {
            printf(" <== rsp");
        }
        printf("\n");
        va -= 8;
    }
}

void TestParsingInstruction()
{
    ACTIVE_CORE = 0x0;    
    core_t *current_active_core = (core_t *)&cores[ACTIVE_CORE];

    char assembly[15][MAX_INSTRUCTION_CHAR] = {
        "push   %rbp",              // 0
        "mov    %rsp,%rbp",         // 1
        "mov    %rdi,-0x18(%rbp)",  // 2
        "mov    %rsi,-0x20(%rbp)",  // 3
        "mov    -0x18(%rbp),%rdx",  // 4
        "mov    -0x20(%rbp),%rax",  // 5
        "add    %rdx,%rax",         // 6
        "mov    %rax,-0x8(%rbp)",   // 7
        "mov    -0x8(%rbp),%rax",   // 8
        "pop    %rbp",              // 9
        "retq",                     // 10
        "mov    %rdx,%rsi",         // 11
        "mov    %rax,%rdi",         // 12
        "callq  0",                 // 13
        "mov    %rax,-0x8(%rbp)",   // 14
    };
    
    inst_t inst;
    for (int i = 0; i < 15; ++ i)
    {
        parse_instruction(assembly[i], &inst, current_active_core);
    }
}

void TestParsingOperand()
{
    ACTIVE_CORE = 0x0;    
    core_t *ac = (core_t *)&cores[ACTIVE_CORE];

    const char *strs[11] = {
        "$0x1234",
        "%rax",
        "0xabcd",
        "(%rsp)",
        "0xabcd(%rsp)",
        "(%rsp,%rbx)",
        "0xabcd(%rsp,%rbx)",
        "(,%rbx,8)",
        "0xabcd(,%rbx,8)",
        "(%rsp,%rbx,8)",
        "0xabcd(%rsp,%rbx,8)",
    };
    
    printf("rax %p\n", &(ac->reg.rax));
    printf("rsp %p\n", &(ac->reg.rsp));
    printf("rbx %p\n", &(ac->reg.rbx));
    
    for (int i = 0; i < 11; ++ i)
    {
        operand_t od;
        parse_operand(strs[i], &od, ac);

        printf("\n%s\n", strs[i]);
        printf("od enum type: %d\n", od.type);
        printf("od imm: %lx\n", od.imm);
        printf("od reg1: %lx\n", od.reg1);
        printf("od reg2: %lx\n", od.reg2);
        printf("od scal: %lx\n", od.scal);
    }
}
